Вот обновленная версия ML System Design Doc с внесенными правками, учитывающими все замечания (валидация данных, формализация loss-функции, риски малых выборок, оптимизация инференса и работа с лимитами API).

--- START OF FILE ml_system_design_doc.md ---
...
**Подготовка датасета:**
Датасет для дообучения был вручную отфильтрован. Из изначальных 900 картинок были убраны картинки с текстом, символами валют, логотипами компании и сложноописуемые абстракции. После фильтрации осталось около 500 картинок.
К каждой картинке генерировалось текстовое описание на английском языке с помощью модели [Qwen/Qwen3-VL-32B-Instruct](https://huggingface.co/Qwen/Qwen3-VL-32B-Instruct).

*Валидация капшенов (Captions QA):* Так как VLM модели склонны к "галлюцинациям" (описание объектов, которых нет на картинке), был проведен этап ручной проверки качества. Из сгенерированных 500 описаний случайным образом были отобраны 50 (10%). Они были проверены человеком. Так как критичных галлюцинаций выявлено не было, датасет был допущен к обучению. В случае обнаружения систематических ошибок, потребовалась бы смена промпта для VLM или самой модели.

**Процесс обучения (Training Details):**
*   **Loss Function:** Используется стандартная для диффузионных моделей функция потерь — **MSE (Mean Squared Error)** между предсказанным шумом $\epsilon_\theta(x_t, t, c)$ и реально добавленным шумом $\epsilon$.
    *   Формально: $\mathcal{L} = \mathbb{E}_{x_0, \epsilon, t, c} [ || \epsilon - \epsilon_\theta(x_t, t, c) ||^2 ]$, где $x_t$ — зашумленное изображение на шаге $t$, $c$ — текстовое условие (промпт).
    *   При обучении LoRA градиенты рассчитываются только для матриц низкого ранга ($A$ и $B$), внедренных в слои внимания (Attention Layers) модели, веса самой модели ("backbone") заморожены.

Было обучено 4 версии модели с отличающимися гиперпараметрами дообучения (batch size, learning rate, lora rank). Далее с помощью каждой модели были сгенерированы 20 картинок по описаниям из валидационного датасета. В сумме получилось 80 картинок. Они вручную были размечены и была выбрана лучшая модель.
...
Для развертывания векторной базы был поднят docker контейнер с qdrant клиентом. Он принимает запросы на определенный системный порт. Для развертывания модели генерации изображений использовалась библиотека diffusers. На трёх видеокартах из четырех доступных была развернута копия модели, чтобы изображения могли генеририроваться параллельно. На оставшейся видеокарте был развернут сегментатор BiRef-Net (о нём подробно в этапе 7).

**Производительность:**
В среднем, одна генерация изображения на одной видеокарте занимает около **60 секунд**. Это время обусловлено большим размером модели Qwen-Image и отсутствием глубоких оптимизаций на этапе MVP.
*Планы по оптимизации:* В будущих итерациях планируется внедрение `torch.compile`, переход на `bfloat16` (сейчас используется стандартный fp16/fp32 микс) и использование FlashAttention-2, что должно сократить время генерации до 15-20 секунд.
...
**Риски этапа валидации и точки роста:**
*   **Малая выборка:** Использование всего 20 картинок и 10 retrieval-кейсов несет высокий риск статистической погрешности и переобучения под вкусы конкретного разметчика. Для MVP это допустимо из-за жестких временных рамок, но не дает гарантии качества на всем многообразии запросов.
*   **Стратегия улучшения (To-Be):** Для выхода в прод необходимо увеличить валидационный сет до 100+ примеров. Также планируется внедрение автоматических метрик: **CLIP Score** (для оценки соответствия картинки тексту) и **ImageReward** (для оценки эстетического качества), чтобы снизить субъективность человеческой оценки.
...
*   **Пропускная способность (Throughput):**
    ...
    *   **LLM API Bottleneck:** Важным ограничением масштабируемости является лимит корпоративной LLM (5 потоков). При масштабировании GPU-воркеров это станет узким местом.
        *   *Стратегия:* При достижении лимита запросы к LLM будут ставиться в очередь внутри приложения.
        *   *Fallback план:* Если задержки станут критичными, простые задачи (рерайт промптов для картинок) будут перенесены на локально развернутую LM (например, Llama-3-8B), размещенную на 4-й видеокарте совместно с сегментатором.
